...
Nan::Persistent<v8::Function> NdtTest::constructor;

NdtTest::NdtTest(v8::Local<v8::Object> options) : options_(options) {
}

NdtTest::~NdtTest() {
}

void NdtTest::Init(v8::Local<v8::Object> exports) {

  // Prepare constructor template
  Nan::HandleScope scope;
  v8::Local<v8::FunctionTemplate> tpl = Nan::New<v8::FunctionTemplate>(New);
  tpl->SetClassName(Nan::New("NdtTest").ToLocalChecked());
  ...
  exports->Set(Nan::New("NdtTest").ToLocalChecked(), tpl->GetFunction());
}


void NdtTest::New(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  if (info.IsConstructCall()) {
    // Invoked as constructor: `new NdtTest(...)`
    ...
    NdtTest* obj = new NdtTest(options);
    obj->Wrap(info.This());
    info.GetReturnValue().Set(info.This());
  } else {
    // Invoked as plain function `NdtTest(...)`, turn into construct call.
    const int argc = 1;
...
void NdtTest::SetOptions(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 2);

  NdtTest* obj = ObjectWrap::Unwrap<NdtTest>(info.Holder());
...
void NdtTest::SetVerbosity(const Nan::FunctionCallbackInfo<v8::Value>& info) {
...
void NdtTest::OnProgress(const Nan::FunctionCallbackInfo<v8::Value>& info) {
...
void NdtTest::OnLog(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 1);
...
void NdtTest::AddInputFilePath(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  // See: https://github.com/nodejs/node-addon-examples/blob/master/3_callbacks/nan/addon.cc
}

void NdtTest::AddInput(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 1);
...
void NdtTest::Run(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 1);
