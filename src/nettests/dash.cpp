...
Nan::Persistent<v8::Function> DashTest::constructor;

DashTest::DashTest(v8::Local<v8::Object> options) : options_(options) {
}

DashTest::~DashTest() {
}

void DashTest::Init(v8::Local<v8::Object> exports) {

  // Prepare constructor template
  Nan::HandleScope scope;
  v8::Local<v8::FunctionTemplate> tpl = Nan::New<v8::FunctionTemplate>(New);
  tpl->SetClassName(Nan::New("DashTest").ToLocalChecked());
  ...
  exports->Set(Nan::New("DashTest").ToLocalChecked(), tpl->GetFunction());
}


void DashTest::New(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  if (info.IsConstructCall()) {
    // Invoked as constructor: `new DashTest(...)`
    ...
    DashTest* obj = new DashTest(options);
    obj->Wrap(info.This());
    info.GetReturnValue().Set(info.This());
  } else {
    // Invoked as plain function `DashTest(...)`, turn into construct call.
    const int argc = 1;
...
void DashTest::SetOptions(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 2);

  DashTest* obj = ObjectWrap::Unwrap<DashTest>(info.Holder());
...
void DashTest::SetVerbosity(const Nan::FunctionCallbackInfo<v8::Value>& info) {
...
void DashTest::OnProgress(const Nan::FunctionCallbackInfo<v8::Value>& info) {
...
void DashTest::OnLog(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 1);
...
void DashTest::AddInputFilePath(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  // See: https://github.com/nodejs/node-addon-examples/blob/master/3_callbacks/nan/addon.cc
}

void DashTest::AddInput(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 1);
...
void DashTest::Run(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  assert(info.Length() >= 1);
